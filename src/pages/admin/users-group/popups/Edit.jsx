...
const EditUserGroup = ({ id, setList, close }) => {
  ...
  useEffect(() => {
    const fetchCategoryData = async () => {
      try {
        const res = await controlPrivateApi.get(`/departments/show/${id}`);
        ...
        if (isAxiosError(error)) {
          notify(
            error.response?.data?.message || "Error fetching user group",
            "error"
          );
        }
      }
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (isAxiosError(error)) {
        notify(
          error.response?.data?.message || "Error updating user group",
          "error"
        );
      }
    } finally {
