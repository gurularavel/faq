...
export default function DifficultyLevels() {
  ...
  const getData = async (queryParams) => {
    ...
    try {
      const res = await controlPrivateApi.get(
        `/difficulty-levels/load?${queryParams}`
      );
  ...
  const deleteRow = async () => {
    try {
      const res = await controlPrivateApi.delete(
        `/difficulty-levels/delete/${draftData?.id}`
      );
      setData((prev) => ({
        ...prev,
  ...
  const popups = [
    "",
    {
      title: t("new_difficulty_level"),
      element: <Add close={() => setModal(0)} setList={setData} />,
    },
    {
      title: t("edit_difficulty_level"),
      element: (
  ...
  return (
    <MainCard title={t("difficulty_levels")}>
      <Modal
        open={open}
